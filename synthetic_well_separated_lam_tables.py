...
"""
synthetic_well_separated_lam_tables.py

Purpose
-------
Generate LaTeX tables for the paper (well_separated synthetic experiments).
This script reads simulation outputs from:
    ./data/{distribution_name}_well_separated.csv
and augments them with two benchmark baselines computed via Monte Carlo:
    1) 'delta'   : the robust min-max benchmark value under lambda (minmax_cost)
    2) 'optimal' : the true (oracle) newsvendor optimum cost (true_cost)

It then produces:
    (A) A LaTeX pivot table of per-algorithm performance gaps:
        - table_additive : additive gap vs appropriate baseline
        - table_relative : percent gap vs appropriate baseline
        The baseline depends on whether the instance is identifiable:
            * if unidentifiable (delta_value > 0), compare to delta/minmax_cost
            * if identifiable (delta_value <= 0), compare to optimal/true_cost
    (B) Counts of identifiability classifications (id metric), grouped by
        algorithm × lambda × param × (b,h).

Key outputs
-----------
- Printed LaTeX tables to stdout (one per distribution and b value).
- Printed classification count tables to stdout.
"""
...
for distribution_name in distribution_list:
    ...
    qbar, param_list, b_list, rho_list, lam_list, gamma = helper.get_well_separated_parameters(distribution_name)

    file_loc = f'./data/{distribution_name}_well_separated.csv'
    ...
    for param in param_values:
        ...
        for b in b_values:
            ...
            for lam in lam_values:
                ...
                print(f'lam: {lam}, Opt value: {mm_opt}, and cost: {mm_cost}')
    ...
    for b in b_values:
        ...
        for (algorithm, lam, param), group in df.groupby(['algorithm', 'lam', 'param']):
            if algorithm != 'delta' and algorithm != 'optimal' and algorithm != 'true value':  # Skip 'delta' and 'optimal' and 'true value' itself
                ...
                # print(f'Lam: {lam}, alg: {algorithm}, identifiable: {identifiable}, delta_value: {delta_value}, alg value: {group.loc[group['metric'] == 'minmax_cost', 'value'].mean()}')
        ...
        print(pivot_table.to_latex(index=True,
                    ...
                    float_format=lambda x: helper.float_format_sig(x, sig=3)
        ))
