...
for distribution_name in distribution_list:
    ...
    algorithm_list = [
        ...
        ('robust', lambda order_levels, censored_demands, b, h: algorithms.robust_saa(order_levels, censored_demands, b, h, qbar, np.mean(gminus_calc_list < lam), delta=0.05)),
        ('km', algorithms.km_estimate),
        ('robust_km', lambda order_levels, censored_demands, b, h: algorithms.robust_km(order_levels, censored_demands, b, h, qbar, np.mean(gminus_calc_list < lam), delta=0.05)),
        ('joint_order_saa', algorithms.joint_order_saa),
        ('robust_plus', lambda order_levels, censored_demands, b, h: algorithms.robust_plus_saa(order_levels, censored_demands, b, h, qbar, np.mean(gminus_calc_list < lam), delta=0.05)),
        ('robust_bonus', lambda order_levels, censored_demands, b, h: algorithms.robust_bonus_saa(order_levels, censored_demands, b, h, qbar, np.mean(gminus_calc_list < lam), delta = 0.05))
    ]
    ...
    for param in param_list: # Loop over each parameter for the distribution type
        ...
        if 'robust' in algorithm_list: # just gotta reset the Gminus estimate for a potential tighter confidence term
            algorithm_list['robust'] = lambda order_levels, censored_demands, b, h: algorithms.robust_saa(order_levels, censored_demands, b, h, qbar, np.mean(gminus_calc_list < lam), delta=0.05)
            algorithm_list['robust_plus'] =  lambda order_levels, censored_demands, b, h: algorithms.robust_plus_saa(order_levels, censored_demands, b, h, qbar, np.mean(gminus_calc_list < lam), delta=0.05),
            algorithm_list['robust_bonus'] = lambda order_levels, censored_demands, b, h: algorithms.robust_bonus_saa(order_levels, censored_demands, b, h, qbar, np.mean(gminus_calc_list < lam), delta = 0.05)
            algorithm_list['robust_km'] = lambda order_levels, censored_demands, b, h: algorithms.robust_km(order_levels, censored_demands, b, h, qbar, np.mean(gminus_calc_list < lam), delta=0.05)
                    ...
                    for _ in range(num_iters):
                        ...
                        for algo_name, algo_func in algorithm_list: # Loops over each algorithm and evaluates it
                            sol, true_cost, mm_cost, ind = helper.evaluate_algorithm(
                                algo_name, algo_func, order_levels, censored_demands, b, h, qbar, eval_demand_list
                            )
