...
for distribution_name in distribution_list:
    ...
    for b in b_values:
        ...
        for N in N_values:
            ...
            # Ensure class columns appear consistently even when absent in a group.
            cat_order = ['identifiable', 'knife_edge', 'unidentifiable']
            classification_df['metric'] = classification_df['metric'].astype('category')
            classification_df['value'] = pd.Categorical(
                classification_df['value'],
            ...
            classification_counts = (
                classification_df.groupby(group_cols + ['value'])
                .size()
                .unstack('value', fill_value=0)
                .reset_index()
            )

            class_cols = [c for c in cat_order if c in classification_counts.columns]
            totals = classification_counts[class_cols].sum(axis=1)
            classification_rates = classification_counts.copy()
            classification_rates[class_cols] = (
                classification_rates[class_cols]
            ...
            # The raw simulation CSV includes non-numeric metric rows (e.g., metric='id'
            # with values like 'identifiable'); coerce to numeric for aggregation.
            df['value'] = pd.to_numeric(df['value'], errors='coerce')

            # Group by 'algorithm', 'lam', 'param', and 'metric', and calculate the average of 'value'
            averaged_df = (
                df.dropna(subset=['value'])
                .groupby(['algorithm', 'lam', 'param', 'metric'])['value']
                .mean()
                .reset_index()
            )
