...
for category in grouped_df['Category'].unique():
    ...
    for table_b in b_values:
        ...
        averaged_df = df.groupby(['algorithm', 'lam', 'metric'])['value'].mean().reset_index()
        ...
        for (algorithm, lam), group in df.groupby(['algorithm', 'lam']):
        ...
        pivot_table = filtered_df.pivot_table(
            index='algorithm', 
            columns=['identifiable', 'lam'], 
            values='value', 
            aggfunc='mean'
        )

        # Sort the columns with 'identifiable' decreasing and 'lam' increasing
        pivot_table = pivot_table.sort_index(axis=1, level=['identifiable', 'lam'], ascending=[True, True])
