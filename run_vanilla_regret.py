...
"""
run_vanilla_regret.py

Purpose
-------
Run Monte Carlo simulations to evaluate *vanilla regret* for censored-demand
newsvendor algorithms, and write a long-format CSV used by downstream plotting
and table scripts.

This script generates:
    ./data/{distribution_name}_vanilla_regret.csv

Experiment design (concise)
---------------------------
For each (distribution, param, b, lam, N):
- Holding cost is fixed at h = 1.
- Each replicate consists of two selling seasons:
    * Season 1: order level q1 = lam (also the censoring threshold).
    * Season 2: order level q2 drawn uniformly from [lam/4, 3lam/4].
- For each season, N i.i.d. demands are drawn and censored as min(D, q).
- Algorithms observe the two censored datasets and return an order decision.
- The decision is evaluated on a large Monte Carlo demand sample to estimate
  expected costs.
...
distribution_list = ['continuous_uniform']
...
for distribution_name in distribution_list:
    ...
    file_name = f'./data/{distribution_name}_vanilla_regret.csv'
